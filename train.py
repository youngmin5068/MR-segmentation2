...
def main(net,device,train_csv,valid_csv):
    ...
    for epoch in range(EPOCH):
        ...
        for imgs, true_masks,true_labels in train_loader:
            ...
            #label_pred
            masks_preds,label_pred = net(imgs)

            loss1 = diceloss(torch.sigmoid(masks_preds),true_masks)
            loss2 = bceloss(label_pred,true_labels)
        ...
        with torch.no_grad():
            ...
            for imgs, true_masks,true_labels in val_loader:
                ...
                thresh_label = torch.zeros_like(label_pred)
                thresh_label[label_pred>0.5] = 1.0

                total += true_labels.size(0)
                correct += (thresh_label == true_labels).sum().item()
           ...
           # mean_bce_loss,acc =0,0
            ...
            if mean_dice_score > best_dice:
                ...
                try:
                    ...
                    pass
                torch.save(net.module.state_dict(), DIR_CHECKPOINT + f'/UNet_classify_result.pth') # d-> custom_UNet_V2 /// e -> att swinUNetr  ////f -> custom unet v2
